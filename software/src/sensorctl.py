...
def red_led(command, *values):
    """
    Turn on and off red light
    :param command: str 'on'|'off
    :return:
    """
    log.info(f"red_led {command},{values}")
...
def speakers(command, *values):
    ...
    log.info(f"Speakers:{command}:{values}")
    if not values:
        tunes = ['lullaby.wav']
    else:
        if values[0]:
...
if __name__ == "__main__":
    ...
    if pargs.sensor == 'temperature':
        ...
        log.info(f"red_led:{red_led(*pargs.parameter)}")
